...
Network::Network(std::vector<NNode *> in,
  ...
  all_nodes = all;
  name =
      0;   //Defaults to no name  ..NOTE: TRYING TO PRINT AN EMPTY NAME CAN CAUSE A CRASH
  numnodes = -1;
  numlinks = -1;
  net_id = netid;
  adaptable = adaptval;
  ...
  for (curnode = all_nodes.begin(); curnode!=all_nodes.end(); ++curnode) {
    if (((*curnode)->type)!=SENSOR) {
      for (curlink = ((*curnode)->incoming).begin(); curlink!=((*curnode)->incoming).end(); ++curlink) {
        oFile << (*curlink)->in_node->node_id << " -> " << (*curlink)->out_node->node_id << " : " << (*curlink)->weight
              << std::endl;
        ...
        if ((*curnode)->active_flag) {
          ...
          if ((*curnode)->overridden()) {
            ...
            if ((*curnode)->ftype==SIGMOID)
              (*curnode)->activation = NEAT::fsigmoid((*curnode)->activesum,
                                                      4.924273,
